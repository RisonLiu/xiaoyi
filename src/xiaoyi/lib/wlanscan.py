...
WlanGetAvailableNetworkList.argtypes = (HANDLE, POINTER(GUID), DWORD, c_void_p, POINTER(POINTER(WLAN_AVAILABLE_NETWORK_LIST)))
        ...
        for iface in ifaces:
            ...
            try:
                ...
                for network in networks2:
                    ...
                    signal_strength = str(network.lRssi)  # 信号强度
                    signal_channel = getChannelByFreq(network.ulChCenterFrequency)  # 信道
                    # print "SSID: " + SSID + " BSSID: "+ BSSID+ " SS: "+signal_strength
                    BSSI_Values[BSSID] = [SSID, signal_strength, str(signal_channel)]
...
import time
def getMacBySsid(listSSID):
    lstSsid = copy.deepcopy(listSSID)
    _listSsid = []
    dictSsid = {}
    for i in range(0, 5):
        dictInfo = get_BSSI()
        for sid in lstSsid:
            for item in dictInfo.items():
                if item[1][0].find(sid) >= 0 and _listSsid.count(item[1][0]) == 0 and dictSsid.keys().count(item[1][0]) == 0:
                    _listSsid.append(item[1][0])
                    if lstSsid.count(sid) > 0:
                        lstSsid.remove(sid)
        for item in dictInfo.items():
            if _listSsid.count(item[1][0]) > 0:
                lstMac = []
                if dictSsid.has_key(item[1][0]):
                    lstMac = dictSsid.get(item[1][0])
                lstMac.append(item[0])
                dictSsid.setdefault(item[1][0], lstMac)
        for item in dictSsid.keys():
            if _listSsid.count(item) > 0:
                _listSsid.remove(item)
        if _listSsid.__len__() == 0 and lstSsid.__len__() == 0:
            break
        time.sleep(5)
    if lstSsid.__len__() > 0:
        print "Warning : ssid not found: %s !!!" % str(lstSsid)
    return dictSsid

def getSignalLevel(listSSid):
    lstSsid = copy.deepcopy(listSSid)
    rtnList = []
    for item in lstSsid:
        rtnList.append("-100")
    dectInfo = get_BSSI()
    for item in dectInfo.items():
        if lstSsid.count(item[1][0]) > 0:
            level = int(item[1][1])
            if level > int(rtnList[lstSsid.index(item[1][0])]):
                rtnList[lstSsid.index(item[1][0])] = item[1][1]
    # print rtnList
    return rtnList

def getChannel(listSSid):
    lstSsid = copy.deepcopy(listSSid)
    rtnList = []
    for item in lstSsid:
        rtnList.append("0")
    ...
    for item in dectInfo.items():
        if lstSsid.count(item[1][0]) > 0:
            if rtnList[lstSsid.index(item[1][0])] == "0":
                rtnList[lstSsid.index(item[1][0])] = item[1][2]
            if ("+"+rtnList[lstSsid.index(item[1][0])]+"+").find("+"+item[1][2]+"+") < 0:
                rtnList[lstSsid.index(item[1][0])] += "+%s" % item[1][2]
    # print rtnList
    return rtnList

if __name__ == '__main__':
    print __name__
    # print getMacBySsid(["002",])
    # print getSignalLevel(["XYDJ_4230002","Xiaomi360"])
    # print getSignalLevel(["XYDJ_4230002",])
    # for item in test.items():
    #     print item
    # print "Count: %d" % test.items().__len__()
    # print get_BSSI()
    # print getChannel(["YDXJ_4714902_5G","YDXJ_4230002_5G","SeeDifferent-5G"])
